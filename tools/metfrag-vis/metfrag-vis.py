...
def cdk_inchi_to_svg(inchi):
    if "cdk-inchi-to-svg" in os.environ:
        JAVA_CMD = 'cdk-inchi-to-svg' + ' ' + str('\'' + inchi + '\'') + ' ' + 'cdk-inchi-to-svg-output.svg'
    else:
        ...
        JAVA_CMD = str(JAVA_BINARY + ' ' + '-jar' + ' ' + CDK_INCHI_TO_SVG_JAR + ' ' + str(
...
def kegg_link(compound_name):
    return (str(
        'https://www.genome.jp/dbget-bin/www_bfind_sub?mode=bfind&max_hit=1000&dbkey=kegg&keywords=' + compound_name))


# ---------- biocyc_link ----------
def biocyc_link(compound_name):
    biocyc_url = urllib.parse.urlparse(
        str('https://www.biocyc.org/substring-search?type=NIL&object=' + compound_name + '&quickSearch=Quick+Search'))
...
def hmdb_link(compound_name):
    hmdb_url = urllib.parse.urlparse(
        str('https://hmdb.ca/unearth/q?utf8=✓&query=' + compound_name + '&searcher=metabolites&button='))
...
def fetch_classyfire_classes(inchi):
    ...
    if (classyfire_request.status_code != 201):
        print('Error! Could not send request to ClassyFire. \"',
              str(classyfire_request.status_code) + ': ' + str(classyfire_request.reason), '\". Skipping entry.')
        return ('&nbsp;')
    ...
    while (classyfire_request_loop < 20):
        ...
        classyfire_query = requests.get(
            str('http://classyfire.wishartlab.com/queries/' + str(classyfire_query_id) + '.json'))
...
def plot_spectrum(spectrum, spectrum_explained, spectrum_explained_formulas):
    ...
    if not (spectrum_explained == 'NA') and not (spectrum_explained_formulas == 'NA'):
        ...
        for i in range(0, len(ex)):
            plt.text(float(ex[i]), float(ey[i]) + 1000, ez[i], color='#2b8126', fontsize=8,
...
metfrag_html.write(
    'body { font-family: Lucida, Verdana, Arial, Helvetica, sans-serif; font-size: 13px; text-align: left; color: #000000; margin: 8px 8px 8px 8px; }\n')
...
metfrag_html.write(
    'h1 { font-size: 32px; font-weight: bold; text-align: center; padding: 0px 0px 4px 0px; margin: 26px 0px 0px 0px; }\n')
metfrag_html.write(
    'h2 { font-size: 24px; font-weight: bold; text-align: left; padding: 0px 0px 4px 0px; margin: 26px 0px 0px 0px; }\n')
metfrag_html.write(
    'table { font-family: Lucida, Verdana, Arial, Helvetica, sans-serif; font-size: 10px; text-align: left; line-height: 10px; border: 1px solid #e3efdf; background-color: #ecf5ea; margin-bottom: 8px; min-width: 1600px; max-width: 2400px; }\n')
...
with open(input_tsv, "r") as metfrag_file:
    ...
    for row in metfrag_results:
        ...
        # Start new document
        if (line_count == 0):
            if os.path.join(os.path.dirname(os.path.abspath(__file__)), 'metfrag_logo.png'):
                logo_pth = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'metfrag_logo.png')
            else:
            ...
            metfrag_html.write(str(
                '\n<h1><img style="vertical-align:bottom" src="data:image/png;base64,' + png_encoded.decode(
                    'utf-8') + '" alt="metfrag-logo" width="150"></img><text style="line-height:2.0">&nbsp;&nbsp;results</text></h1>\n'))
        else:
            # Parameter list at beginning of document
            if (line_count == 1):
                metfrag_html.write('\n<h2>Parameter list</h2>\n')
                metfrag_html.write(str('MetFragDatabaseType=' + re.sub(' .*', '', re.sub('.*MetFragDatabaseType=', '',
                                                                                         row[
                                                                                             "MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('PrecursorIonMode=' + re.sub(' .*', '', re.sub('.*PrecursorIonMode=', '', row[
                    "MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('DatabaseSearchRelativeMassDeviation=' + re.sub(' .*', '', re.sub(
                    '.*DatabaseSearchRelativeMassDeviation=', '', row["MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('FragmentPeakMatchAbsoluteMassDeviation=' + re.sub(' .*', '', re.sub(
                    '.*FragmentPeakMatchAbsoluteMassDeviation=', '', row["MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('FragmentPeakMatchRelativeMassDeviation=' + re.sub(' .*', '', re.sub(
                    '.*FragmentPeakMatchRelativeMassDeviation=', '', row["MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('FilterExcludedElements=' + re.sub(' .*', '',
                                                                          re.sub('.*FilterExcludedElements=', '',
                                                                                 row["MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('FilterIncludedElements=' + re.sub(' .*', '',
                                                                          re.sub('.*FilterIncludedElements=', '',
                                                                                 row["MetFragCLIString"])) + '<br>\n'))
                metfrag_html.write(str('MetFragScoreTypes=' + re.sub(' .*', '', re.sub('.*MetFragScoreTypes=', '', row[
                    "MetFragCLIString"])) + '<br>\n'))
            ...
            if (row["name"] != compound):
                ...
                metfrag_html.write(
                    str('<b>Precursor Retention Time:</b> ' + str(round(float(precursor_rt), 4)) + '<br></p>'))
                metfrag_html.write(str('\n' + '<table>\n'))
            ...
            if (candidates < max_candidates):
                ...
                # External links
                external_links = str(
                    '<a target="_blank" href="' + pubchem_link(compound_name) + '">PubChem</a>' + ', ' +
                ...
                # MetFragWeb
                FragmentPeakMatchAbsoluteMassDeviation = str('' + re.sub(' .*', '', re.sub(
                    '.*FragmentPeakMatchAbsoluteMassDeviation=', 'FragmentPeakMatchAbsoluteMassDeviation=',
                    row["MetFragCLIString"])))
                FragmentPeakMatchRelativeMassDeviation = str('' + re.sub(' .*', '', re.sub(
                    '.*FragmentPeakMatchRelativeMassDeviation=', 'FragmentPeakMatchRelativeMassDeviation=',
                    row["MetFragCLIString"])))
                DatabaseSearchRelativeMassDeviation = str('' + re.sub(' .*', '',
                                                                      re.sub('.*DatabaseSearchRelativeMassDeviation=',
                                                                             'DatabaseSearchRelativeMassDeviation=',
                                                                             row["MetFragCLIString"])))
                IonizedPrecursorMass = str('IonizedPrecursorMass=' + str(row["precursor_mz"]))
                NeutralPrecursorMass = str('' + re.sub(' .*', '',
                                                       re.sub('.*NeutralPrecursorMass=', 'NeutralPrecursorMass=',
                                                              row["MetFragCLIString"])))
                NeutralPrecursorMolecularFormula = str(
                    'NeutralPrecursorMolecularFormula=' + str(row["MolecularFormula"]))
                PrecursorIonMode = str(
                    '' + re.sub(' .*', '', re.sub('.*PrecursorIonMode=', 'PrecursorIonMode=', row["MetFragCLIString"])))
                PeakList = str(
                    '' + re.sub(' .*', '', re.sub('.*PeakListString=', 'PeakList=', row["MetFragCLIString"])))
                MetFragDatabaseType = str('' + re.sub(' .*', '',
                                                      re.sub('.*MetFragDatabaseType=', 'MetFragDatabaseType=',
                                                             row["MetFragCLIString"])))
